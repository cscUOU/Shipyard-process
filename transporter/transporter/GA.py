...
def evaluation(arr):
    ...
    for i in range(len(arr.works)):
        ...
        for start, end in times:
            if not (pick_up_time >= end or finish_time <= start) or finish_time > 18 or arr.works[
                i].end_time < finish_time or finish_time + 0.5 > 18:
                flag = False
                break
        if flag:
    ...
    for _ in range(size):
        ...
        for block in blocks:
            ...
            while candidates:
                ...
                temp_trans.works.insert(random.randint(0, max(1, len(temp_trans.works) - 1)), block)
